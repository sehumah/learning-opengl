...
int main () {
    ...
    GLFWwindow* window = glfwCreateWindow(800, 600, "Hello Triangle - LearnOpenGL", NULL, NULL);
    if (window == NULL) {
    ...
    // 12. setup vertex data (vertices for the triangle)
    float vertices[] = {
        -0.5f, -0.5f, 0.0f, // left  
    ...
    // 13. define & bind the vertex array object. Always generate & bind the VAO first, before generating and configuring the VBO(s)
    unsigned int VAO;
     ...
     */
    // 14.then bind and set the vertex buffer(s), and then configure vertex attributes(s) i.e. copy the vertices array in a vertex buffer (VBO) for OpenGL to use
    unsigned int VBO;
    glGenBuffers(1, &VBO);
    ...
    while (!glfwWindowShouldClose(window)) {
         ...
         */
        glDrawArrays(GL_TRIANGLES, 0, 3);  // used when there's only a VBO defined
